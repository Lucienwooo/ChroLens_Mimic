...
def client_to_screen(hwnd, x, y, window_name=""):
    try:
        left, top, right, bottom = win32gui.GetWindowRect(hwnd)
        abs_x = left + x
        abs_y = top + y
...
class RecorderApp(tb.Window):
    ...
    # 新增：儲存使用者設定的方法（避免 AttributeError）
    def save_config(self):
        try:
            save_user_config(self.user_config)
        except Exception as ex:
            try:
                self.log(f"[{format_time(time.time())}] 儲存設定失敗: {ex}")
            except Exception:
                # 若 log 尚未可用，則安靜失敗
                pass
    ...
    def start_record(self):
        # 將 UI 的 start_record 直接委派給 recorder
    ...
    def play_record(self, event=None):
        # 防護：若 recorder 未初始化，記錄並返回
        recorder = getattr(self, "recorder", None)
        if recorder is None:
            try:
                self.log(f"[{format_time(time.time())}] play_record: recorder 尚未初始化")
            except Exception:
                pass
            return

        # 確保 recorder 有目前事件
        recorder.events = getattr(self, "events", [])
        try:
            speed_val = int(self.speed_var.get())
        except:
            speed_val = 100
        speed = max(1, min(1000, speed_val)) / 100.0
        try:
            repeat = int(self.repeat_var.get())
        except:
            repeat = 1
        recorder.play(speed=speed, repeat=repeat, on_event=lambda e: self.log(f"play event: {e}"))
        self.playing = True
        self.after(100, self._update_play_time)

    def auto_save_script(self):
        # 用 ScriptManager auto_save
        try:
            meta = {"speed": self.speed_var.get(), "repeat": self.repeat_var.get()}
            fname = self.script_manager.auto_save(self.events, meta=meta)
            self.log(f"[{format_time(time.time())}] 自動存檔：{fname}")
            self.refresh_script_list()
            self.script_var.set(fname)
